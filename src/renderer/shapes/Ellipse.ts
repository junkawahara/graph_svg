...
export class Ellipse implements Shape {
  ...
  static fromBoundingBox(topLeft: Point, bottomRight: Point, style: ShapeStyle): Ellipse {
    const cx = Math.round((topLeft.x + bottomRight.x) / 2);
    const cy = Math.round((topLeft.y + bottomRight.y) / 2);
    const rx = Math.round(Math.abs(bottomRight.x - topLeft.x) / 2);
    const ry = Math.round(Math.abs(bottomRight.y - topLeft.y) / 2);
  ...
  static fromCenter(center: Point, rx: number, ry: number, style: ShapeStyle): Ellipse {
    return new Ellipse(generateId(), Math.round(center.x), Math.round(center.y), Math.round(rx), Math.round(ry), style);
  }
