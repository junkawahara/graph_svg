...
### 修正
- エッジのラベルが表示されない問題を修正
  - 既存のエッジにラベルを後から追加した場合にラベル要素が作成されない問題を修正
  - `updateElement()` メソッドでラベル要素の動的な追加・削除に対応
- Electronアプリのメインメニューに「グラフ」メニューを追加
  - グラフファイルをインポート...
  - グラフをエクスポート...
  - ノードを自動ラベル
  - エッジを自動ラベル
...
### 追加
- グラフファイルエクスポート機能
  - Edge List形式とDIMACS形式でグラフをエクスポート可能
  - 「Graph」メニュー > 「グラフをエクスポート...」からアクセス
  - **Edge List形式**
    - 1行1エッジ、スペース区切り（source target [label]）
    - エッジラベルがあれば3列目に出力
    - 孤立ノード（エッジに接続されていないノード）は出力しない
  - **DIMACS形式**
    - ヘッダー: `p n m`（n: ノード数、m: エッジ数）
    - コメント行でノード番号とラベルの対応を出力（`c 1: A`）
    - エッジ: `e src dst`（番号で指定）
    - ノードは作成順に1から番号付け
    - 孤立ノードも頂点数に含める
    - エッジラベルは出力しない
...
- Auto Label Nodes 機能
  ...
  - ノードがない場合は静かに無視
- Auto Label Edges 機能
  - エッジを作成順（内部ID順）に自動ラベル付け（例: e0, e1, e2, ...）
  - 既存ラベルを上書き
  - Undo/Redo 対応
  - エッジがない場合は静かに無視
...
- 設定画面に Auto Edge Label 設定を追加
  - Auto Edge Label Prefix（デフォルト: `e`、空文字可）
  - Auto Edge Label Start Number（デフォルト: `0`、負の数可、整数のみ）
- エッジラベルの auto 配置機能
  - デフォルト位置を `auto` に設定
  - `auto` は他のオブジェクトと重ならない位置に自動配置
    - 長さ方向（pos）: 0.5, 0.4, 0.6, 0.3, 0.7, 0.25, 0.75 を試行
    - 側面（side）: above, below を試行
    - 距離（distance）: 基準値, +3px, +6px, +9px を試行
    - フォールバック: 重なりが最も少ない位置に配置
  - 重なり判定対象: ノード（楕円）、ノードラベル、エッジ、エッジラベル、通常図形
  - 計算順序: ノードラベル → エッジラベル（ID順）
  - 再計算トリガー: `history:changed` イベント（ノードと同じ）
  - SVG 保存時は `data-label-position="auto"` として保存
  - サイドバー: Auto 選択時は side/distance セクションを非表示
  - `AutoEdgeLabelPlacementManager` による効率的なバッチ計算
- ノードの auto ラベル配置機能
